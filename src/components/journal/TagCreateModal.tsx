    ...
    try {
      if (selectedFile && currentUserId) {
        ...
        if (uploadedPath) {
          ...
          if (!imageUrlCached) {
            console.warn(
              `Failed to get public URL for ${imagePath}, but upload was successful. Proceeding with path only for image_url_cached.`
            );
          }
        } else {
          console.error(
            "Upload successful but path is missing in response data from uploadFile service."
          );
          throw new Error(
            "Storage upload failed: path missing in response from service."
          );
        }
      }
        ...
        >
            ...
            {showIconPicker && (
              <LucideIconPicker
                onSelectIcon={(selectedName) => {
                  setIconName(selectedName as LucideIconName);
                  setShowIconPicker(false);
                }}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              If image is uploaded, icon/emoji will be ignored.
            </p>
          </div>
          <div>
            <label
