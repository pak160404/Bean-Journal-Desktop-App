...
const RecentCards: React.FC<RecentCardsProps> = ({ tags, onAddNewTag, onEditTag, onDeleteTag, supabase }) => {
                    ...
                    {tags.map((tag) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => onEditTag(tag)} // Allow clicking whole card to edit
                            >
                                ...
                                <div
                                    className="h-3/5 w-full rounded-t-2xl flex items-center justify-center p-0 transition-all duration-200 group-hover:opacity-90 relative overflow-hidden"
                                    style={!imageUrl ? getGradientStyle(tag.color_hex) : { backgroundColor: tag.color_hex || '#e0e0e0' } }
                                >
                                    {imageUrl ? (
                                        <img src={imageUrl} alt={tag.name} className="w-full h-full object-cover" />
                                    ) : tag.icon_emoji ? (
                                        <span className="text-4xl md:text-5xl" role="img" aria-label={tag.name}>{tag.icon_emoji}</span>
                                    ) : tag.icon_name ? (
                                        // Placeholder for icon_name rendering. Could be a component or class-based icon.
                                        // For now, just display the name, styled differently.
                                        <span className="text-white text-xl font-semibold p-2 break-all text-center opacity-80 group-hover:opacity-100 drop-shadow-sm">{tag.icon_name}</span>
                                    ) : (
